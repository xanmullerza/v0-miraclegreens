...
interface ChatbotImportViewProps {
    ...
    // Props from the old import view that we might still need for full integration
...
export function ChatbotImportView({
    ...
    // If we have a success recipe, show the review/save view
    if (successRecipe && handleSaveAndViewRecipe && setSuccessRecipe) {
        return (
            <div className="flex-1 overflow-y-auto custom-scrollbar p-6 flex flex-col animate-in fade-in zoom-in-95 duration-300">
                <div className="bg-emerald-500/10 dark:bg-emerald-500/20 rounded-3xl p-8 border border-emerald-500/20 text-center mb-6">
                    <div className="w-20 h-20 rounded-2xl bg-emerald-500 text-white flex items-center justify-center mx-auto mb-6 shadow-lg shadow-emerald-500/20 animate-bounce">
                        <FileText size={40} />
                    </div>
                    <h3 className="text-xl font-black text-slate-900 dark:text-white mb-2 uppercase tracking-tight">Recipe Parsed!</h3>
                    <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">We've successfully extracted "{successRecipe.title}". Would you like to save it to your library or make adjustments?</p>
                </div>
                ...
                <div className="space-y-3">
                    <button
                        ...
                        className="w-full h-14 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-black uppercase tracking-[0.2em] transition-all active:scale-95 shadow-xl shadow-emerald-600/20 flex items-center justify-center gap-2 disabled:opacity-50"
                    >
                        {recipeSaving ? <Loader2 size={18} className="animate-spin" /> : <span>SAVE TO LIBRARY</span>}
                    </button>
                    <button
                        onClick={() => setSuccessRecipe(null)}
                        className="w-full h-14 rounded-2xl bg-secondary text-secondary-foreground hover:bg-muted font-black uppercase tracking-[0.2em] transition-all active:scale-95"
                    >
                        TRY ANOTHER
                    </button>
                </div>
    ...
    return (
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 flex flex-col animate-in fade-in duration-300">
            {/* Header */}
            <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                    <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Add New Recipe</h3>
                </div>
                <button
                    onClick={() => setChatbotView('cookbook')}
                    className="group flex items-center gap-2 px-3 py-1.5 rounded-xl bg-secondary text-secondary-foreground hover:bg-muted transition-all"
                >
            ...
            <div className="grid grid-cols-2 gap-3 w-full mb-4">
                ...
                <div className="bg-emerald-500/10 dark:bg-emerald-500/5 rounded-2xl p-4 border border-emerald-500/20 flex flex-col h-full relative overflow-hidden group hover:border-emerald-500/40 transition-all duration-300">
                    ...
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 mb-4 flex-1">
                        Step-by-step form to add all details yourself.
                    </p>
                    <button
                ...
                <div className="bg-blue-500/10 dark:bg-blue-500/5 rounded-2xl p-4 border border-blue-500/20 flex flex-col h-full relative overflow-hidden group hover:border-blue-500/40 transition-all duration-300">
                    ...
                    >
                        {isLoading || recipeLoading ? <Loader2 size={12} className="animate-spin" /> : <span>✓ Parse & Review</span>}
                    </button>
                </div>
                ...
                <div className="bg-purple-500/10 dark:bg-purple-500/5 rounded-2xl p-4 border border-purple-500/20 flex flex-col h-full relative overflow-hidden group hover:border-purple-500/40 transition-all duration-300">
                    ...
                    <button
                        ...
                        disabled={!pastedRecipeURL.trim() || isLoading || recipeLoading}
                    ...
                    >
                        {isLoading || recipeLoading ? <Loader2 size={12} className="animate-spin" /> : <span>✓ Import Recipe</span>}
                    </button>
                </div>
                ...
                >
                    ...
                    <div className="flex flex-col items-center justify-center py-4 bg-slate-100/50 dark:bg-slate-800/50 rounded-xl border border-rose-500/10 transition-colors group-hover:bg-rose-500/10">
                        {isLoading || recipeLoading ? (
                             <Loader2 size={20} className="text-rose-500 animate-spin mb-2" />
                        ) : (
                            <>
                    ...
                    <input
                        type="file"
                        ref={fileInputRef}
                        onChange={(e) => e.target.files?.[0] && processRecipeImage(e.target.files[0])}
                        accept="image/*"
                        className="hidden"
                    />
                </div>
                ...
                <div className="bg-cyan-500/10 dark:bg-cyan-500/5 rounded-2xl p-4 border border-cyan-500/20 flex flex-col h-full relative overflow-hidden group hover:border-cyan-500/40 transition-all duration-300">
                    ...
                    <button
                        ...
                        disabled={!videoURL.trim() || isLoading || recipeLoading}
                    ...
                    >
                        {isLoading || recipeLoading ? <Loader2 size={12} className="animate-spin" /> : <span>✓ Import Video</span>}
                    </button>
                </div>
            </div>
            ...
            {/* Bulk Import Button */}
            <button
                onClick={() => toast('Bulk database import is coming soon!')}
                className="w-full p-4 rounded-2xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 flex items-center justify-between group hover:scale-[1.01] transition-all duration-300 shadow-xl overflow-hidden relative"
            >
