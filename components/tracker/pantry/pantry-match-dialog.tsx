...
interface PantryMatchDialogProps {
    ...
    initialQuery: string;
    onClose: () => void;
    onConfirm: (foodItemId: string | null, name: string, quantity: string) => void;
    knownItem?: {
        name: string;
        image?: string;
        quantity?: string;
...
export function PantryMatchDialog({ isOpen, initialQuery, onClose, onConfirm, knownItem }: PantryMatchDialogProps) {
    ...
    useEffect(() => {
        if (isOpen) {
            if (knownItem) {
                // Confirm mode
                setQuery(knownItem.name);
                setQuantity(knownItem.quantity || '1');
            } else {
                // Search mode
    ...
    }, [isOpen, initialQuery, knownItem]);

    // Search effect (only if not knownItem)
    useEffect(() => {
        if (knownItem) return;
    ...
    }, [debouncedQuery, isOpen, knownItem]);

    const handleConfirm = () => {
        if (knownItem) {
            // Pass null ID for known items (logic handled in parent) or we could pass ID if we had it but parent has it
            onConfirm(null, knownItem.name, quantity);
        } else {
    ...
    return (
        <Sheet open={isOpen} onOpenChange={onClose}>
            <SheetContent side="bottom" className="sm:max-w-md bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 h-[85vh] sm:h-auto rounded-t-[2rem] mx-auto">
                <SheetHeader>
                    <SheetTitle className="text-xl font-black uppercase tracking-tight text-slate-900 dark:text-white">
                        {knownItem ? 'Confirm Pantry Item' : 'Match Pantry Item'}
                    </SheetTitle>
                    <SheetDescription>
                        {knownItem
                            ? `Verify quantity for "${knownItem.name}" before adding.`
                            : `Link "${initialQuery}" to our database.`}
                    </SheetDescription>
                </SheetHeader>
                ...
                <div className="space-y-4 py-4">
                    ...
                    {knownItem && (
                        <div className="flex items-center gap-4 p-4 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
                            <div className="w-16 h-16 rounded-xl bg-white dark:bg-slate-900 flex items-center justify-center overflow-hidden shrink-0 border border-slate-100 dark:border-slate-800">
                                {knownItem.image ? (
                                    <img src={knownItem.image} alt={knownItem.name} className="w-full h-full object-cover" />
                                ) : (
                                    <Check size={24} className="text-emerald-500" />
                                )}
                            </div>
                            <div>
                                <h4 className="font-bold text-slate-900 dark:text-white">{knownItem.name}</h4>
                                <p className="text-xs text-slate-500">Ready to add</p>
                            </div>
                        </div>
                    )}

                    {/* Search Input (Hidden if known) */}
                    {!knownItem && (
                ...
                <SheetFooter className="gap-2">
                    ...
                    >
                        {knownItem ? 'Add to Pantry' : 'Confirm Match'}
                    </Button>
                </SheetFooter>
            </SheetContent>
        </Sheet>
