  ...
  return (
    <div className="group relative bg-card rounded-2xl border border-border overflow-hidden hover:shadow-lg transition-all animate-in fade-in zoom-in-95 duration-500 flex flex-col h-full">
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        <Link href={`/recipes/${recipe.id}`} className="mt-auto block w-full">
        ...
        {onRegenerate && (
          <button
            className="mt-2 w-full py-2 px-4 bg-muted hover:bg-muted/80 text-muted-foreground rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2 relative z-20"
            onClick={(e) => {
              e.stopPropagation();
              onRegenerate();
            }}
    ...
    >
      ...
      <div className="lg:grid lg:grid-cols-[100px_1fr_60px_60px_60px_60px_240px] gap-1.5 lg:items-center lg:px-6">
        ...
        <div className="aspect-[4/3] lg:aspect-square w-full lg:w-24 rounded-xl lg:rounded-none bg-muted overflow-hidden relative">
          ...
          <div className="absolute top-1 left-1 bg-black/60 text-white text-[7px] font-bold px-1.5 py-0.5 rounded-full backdrop-blur-sm uppercase">
            {mealLabel}
          </div>
        </div>
        ...
        <div className="p-1 lg:p-0">
          ...
          {(() => {
            ...
            const colors = {
              green: {
                btn: 'border-emerald-500/30 bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500',
              },
              blue: { btn: 'border-blue-500/30 bg-blue-500/10 text-blue-500 hover:bg-blue-500' },
              orange: {
                btn: 'border-amber-500/30 bg-amber-500/10 text-amber-500 hover:bg-amber-500',
              },
            };
            const c = colors[tier];
            return (
              <div className="mt-2 grid grid-cols-2 gap-1 lg:hidden">
                <button
                  onClick={(e: React.MouseEvent) => {
                    e.stopPropagation();
                    setActivePanel(activePanel === 'stocked' ? null : 'stocked');
                  }}
                  className={cn(
                    ...
                    c.btn,
                    activePanel === 'stocked' && 'ring-2 ring-offset-1 ring-current',
                  )}
                >
                ...
                {toBuyState === 'ready' ? (
                  ...
                  <button
                    onClick={(e: React.MouseEvent) => {
                      e.stopPropagation();
                      setActivePanel(activePanel === 'toBuy' ? null : 'toBuy');
                    }}
                    className={cn(
                      'text-[9px] font-black uppercase tracking-widest px-2.5 py-1.5 rounded-lg border hover:text-white transition-all flex items-center justify-center gap-1.5',
                      toBuyState === 'toBuy'
                  ...
                  >
                    {toBuyState === 'toBuy' ? (
                      ...
                      <>
                        <ShoppingBasket size={10} />
                        {uniqueMissing.length} / {recipeIngs.length} To Add
                      </>
                    )}
                  </button>
                )}
      ...
      {activePanel && (
        ...
        >
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            >
              <Plus size={10} />{' '}
              {addedToList ? 'Added to Shopping List' : 'Add All to Shopping List'}
            </button>
          )}
        </div>
      )}
...
export default function MealPlannerContent({
  ...
  const handleMarkEaten = async (recipe: Recipe, mealType: string) => {
    ...
    console.log(
      '[markEaten] Ingredients:',
      ingredients.map((i) => ({
        item: i.item,
        baseIngredient: i.baseIngredient,
        food_item_id: i.food_item_id,
        weightG: i.weightG,
      })),
    );
    console.log(
      '[markEaten] Pantry items:',
      pantryItems.length,
      pantryItems.map((p) => ({ id: p.id, name: p.name, common_name: p.common_name })),
    );
    ...
    for (const ing of ingredients) {
      ...
      if (ing.food_item_id && quantities[ing.food_item_id] !== undefined) {
        ...
        console.log(
          '[markEaten] Direct food_item_id match in localStorage:',
          ing.item,
          '->',
          itemId,
        );
      ...
      if (!itemId && ing.food_item_id) {
        ...
        if (pantryMatch) {
          ...
          console.log(
            '[markEaten] food_item_id matched pantry item:',
            ing.item,
            '->',
            pantryMatch.name || pantryMatch.common_name,
          );
        }
      }
      ...
      if (!itemId) {
        ...
        const matchItem = pantryItems.find((p) => {
          ...
          const splitWords = (str: string): Set<string> =>
            new Set<string>(
              str
                .replace(/,/g, '')
                .split(/\s+/)
        ...
        if (matchItem) {
          itemId = matchItem.id;
          console.log(
            '[markEaten] Name-matched:',
            ing.item,
            '->',
            matchItem.name || matchItem.common_name,
            '(id:',
            matchItem.id,
            ')',
          );
    ...
    if (subtracted > 0) {
      ...
      const unmatched = ingredients
        .filter((i) => (i.weightG ?? 0) > 0)
        .map((i) => i.item)
        .join(', ');
      console.log('[markEaten] No items subtracted. Unmatched ingredients:', unmatched);
      toast.success(
        `${recipe.title} marked as eaten (no pantry items matched: ${unmatched || 'none had weight'})`,
      );
    }
  };
  ...
  const [expandedBreakdownSections, setExpandedBreakdownSections] = useState<
  ...
  const setShowFavoritesOnly =
    externalSetShowFavoritesOnly !== undefined
      ? externalSetShowFavoritesOnly
      : setLocalShowFavoritesOnly;
  const selectedTypes =
    externalSelectedTypes !== undefined ? externalSelectedTypes : localSelectedTypes;
  ...
  > = {
    'Vitamin A': [
      ...
      {
        label: 'Beta-cryptoxanthin',
        keys: ['Beta-cryptoxanthin', 'beta_cryptoxanthin_ug'],
        unit: '�g',
      },
      {
        label: 'Lutein+Zeaxanthin',
        keys: ['Lutein+Zeaxanthin', 'lutein_zeaxanthin_ug'],
        unit: '�g',
      },
    ...
    'Vitamin E': [
      {
        ...
        keys: ['Vitamin E', 'vitamin_e_mg', 'alpha_tocopherol_mg'],
        unit: 'mg',
      },
    ...
    Protein: [
      ...
      {
        label: 'Phenylalanine',
        keys: ['Phenylalanine', 'phenylalanine_g'],
        unit: 'g',
        isEssential: true,
      },
    ...
    Carbs: [
      ...
      {
        label: 'Added Sugars',
        keys: ['Added Sugars', 'added_sugars_g'],
        unit: 'g',
    ...
    Fat: [
      ...
      {
        label: 'Omega-3',
        keys: ['Omega-3', 'omega3_g', 'omega_3_g'],
        unit: 'g',
        isExpandable: true,
      },
  ...
  const handleNextStep = () => {
    ...
    if (goal === 'lose-fat') tdee *= 0.8;
    if (goal === 'build-muscle') tdee *= 1.1;
  ...
  const handleRegenerateMeal = async (
    ...
    if (!plan) return;
    const result = await getRandomRecipeByType(
      mealType,
      diet,
      currentId,
      showFavoritesOnly,
      ...
      >
        ...
        {pageMode === 'maker' && (
          <div className="fixed inset-0 z-50 bg-slate-50 dark:bg-slate-950 animate-in fade-in duration-300 flex flex-col">
            ...
            {/* Maker Options */}
            {makerMode === 'menu' && (
              <div className="flex-1 overflow-y-auto p-8 flex items-start justify-center pt-4">
          ...
          <HeaderActions>
            <div className="flex items-center gap-2">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent
                  align="end"
                  className="w-56 p-2 rounded-2xl border-slate-200 dark:border-slate-800 shadow-2xl bg-white dark:bg-slate-950"
                >
                  {/* Favorites Section */}
            ...
            <div className="space-y-8 max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4">
              <div className="text-center space-y-2">
                <h2 className="text-3xl font-bold">
                  Welcome back, {profile.nickname || profile.name || 'Researcher'}
                </h2>
                <p className="text-slate-500">
                  We've loaded your saved health and dietary preferences.
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                  ...
                  <p className="font-bold">
                    {profile.age}y � {profile.weight}
                    {measurementUnit === 'metric' ? 'kg' : 'lb'} � {profile.height}
                    {measurementUnit === 'metric' ? 'cm' : 'ft'}
                  </p>
                </div>
                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                  <p className="text-[10px] uppercase font-black tracking-widest text-slate-400 mb-1">
              ...
              <div className="space-y-4">
                <Button
                  ...
                  className="w-full h-14 text-lg font-bold rounded-2xl shadow-lg shadow-emerald-500/20"
                >
                ...
                <div className="flex items-center justify-between px-2">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer group">
                    ...
                    <span className="text-xs font-medium text-slate-500 group-hover:text-slate-700 dark:group-hover:text-slate-300 transition-colors">
                      Always skip this summary
                    </span>
                  </label>
                </div>
              </div>
            ...
            <div className="flex flex-col items-center justify-center py-10 text-center space-y-6 animate-in fade-in slide-in-from-bottom-4 max-w-xl mx-auto">
              ...
              <div className="space-y-3">
                ...
                <p className="text-slate-500 font-medium">
                  To curate a personalised meal plan, we need some information from you. This
                  includes your age, weight, height, and diet goals. This is stored locally on your
                  device and not on our servers. You are welcome to skip this and have a look at our
                  recipes for some ideas on your next meal. We value and respect your privacy.
                </p>
              </div>
              ...
              {!generating ? (
                <div className="space-y-6 max-w-sm">
                  ...
                  <p className="text-muted-foreground">
                    Roughly <strong>{formatEnergy(calories, unit)}</strong> across 3 meals.
                  </p>
                  <Button size="lg" onClick={handleGenerate} className="w-full h-14 text-lg">
                    Generate Menu
                  </Button>
                  <Button
                    variant="ghost"
                ...
                <div className="space-y-4 animate-pulse">
                  ...
                  <h3 className="text-xl font-bold">Curating...</h3>
                </div>
              )}
            </div>
          )}
            ...
            <div className="space-y-6 animate-in fade-in-up duration-500">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                {(() => {
                  ...
                  let off = 0;
                  const carbsSeg = seg(off, carbsFrac);
                  off += carbsFrac;
                  const fatSeg = seg(off, fatFrac);
                  off += fatFrac;
                  const proteinSeg = seg(off, proteinFrac);

                  const fmt = (v: number) =>
                    v >= 1000 ? `${(v / 1000).toFixed(1)}k` : v % 1 === 0 ? `${v}` : v.toFixed(1);
                      ...
                      <div className="space-y-2.5">
                        {[
                          ...
                          <div key={label} className="flex items-center gap-2.5">
                            ...
                            <span
                              className={cn(
                                'text-[10px] font-black w-7 text-right flex-shrink-0',
                                textColor,
                              )}
                            >
                              {pct}%
                            ...
                            <span className="text-[10px] text-slate-400 whitespace-nowrap flex-shrink-0">
                              {val.toFixed(1)}g / {rda.toFixed(0)}g
                            </span>
                          </div>
                        ))}
                      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            ...
                            <div key={label}>
                              ...
                              {rda > 0 && (
                                <div className="text-[9px] text-slate-300 dark:text-slate-600 mt-0.5">
                                  target {rda % 1 === 0 ? rda : rda.toFixed(1)}
                                  {unit}
                                  {label === 'Water' ? ' (from food est.)' : ' RDA'}
                                </div>
                              )}
                            </div>
                          );
                ...
                {(() => {
                  ...
                  const micro = plan.micronutrients || {};
                  const vitaminData = ALL_VITAMINS.map(
                    ({ label, fullName, subtitle, excludeFromCount }) => {
                      let val = 0;
                        ...
                        {vitaminData.map(({ label, fullName, subtitle, pct, excludeFromCount }) => {
                                ...
                                {b7InfoOpen && (
                                  <div className="col-span-2 rounded-lg bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/50 p-2.5 text-[9px] text-amber-800 dark:text-amber-300 leading-relaxed">
                                    <p className="font-black uppercase tracking-wide mb-1">
                                      Why isn't B7 (Biotin) tracked?
                                    </p>
                                    <ul className="space-y-0.5 list-none">
                                      <li>
                                        • Biotin is <strong>everywhere</strong> in food — deficiency
                                        is extremely rare in healthy people.
                                      </li>
                                      <li>
                                        • Your gut bacteria{' '}
                                        <strong>synthesise meaningful amounts</strong> of it
                                        continuously, independent of diet.
                                      </li>
                                      <li>
                                        • It has{' '}
                                        <strong>no established Tolerable Upper Intake Level</strong>{' '}
                                        — it's that safe.
                                      </li>
                                      <li>
                                        • Nutrition databases have{' '}
                                        <strong>incomplete biotin data</strong>, so any figure would
                                        be unreliable.
                                      </li>
                                      <li>
                                        • For these reasons it's{' '}
                                        <strong>rarely printed on food labels</strong> and excluded
                                        from most dietary tracking tools.
                                      </li>
                                    </ul>
                                  </div>
                                )}
                          ...
                          return (
                            ...
                            >
                              ...
                              <span className={cn('text-[10px]', hit ? 'font-black' : '')}>
                                {pct}%
                              </span>
                            </div>
                          );
              ...
              <div className="grid grid-cols-3 gap-3 pt-2">
                ...
                <button
                  onClick={() =>
                    setNutrientView(nutrientsView === 'closed' ? 'essential' : 'closed')
                  }
                  className={cn(
                      ...
                      const NutrientGrid = ({
                        ...
                        const themes = {
                          ...
                          rose: {
                            ...
                            text: 'text-rose-400',
                            border: 'border-slate-800',
                            itemBorder: 'border-rose-900/50',
                          },
                          orange: {
                            bg: 'bg-slate-900 border-slate-800',
                            text: 'text-orange-400',
                            border: 'border-slate-800',
                            itemBorder: 'border-orange-900/50',
                          },
                          emerald: {
                            bg: 'bg-slate-900 border-slate-800',
                        ...
                        return (
                          <div
                            className={cn(
                              'p-6 pt-5 rounded-3xl border bg-gradient-to-br mb-6',
                              t.bg,
                            )}
                          >
                            <h4
                              className={cn(
                                'font-black flex items-center gap-2 mb-1 uppercase tracking-widest text-[10px]',
                                t.text,
                              )}
                            >
                            ...
                            {subtitle && (
                              <p
                                className={cn(
                                  'text-[9px] text-slate-400 mb-4 border-b pb-2 transition-colors',
                                  t.border,
                                )}
                              >
                                {subtitle}
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
                              {Object.entries(items).map(([label, keys]) => {
                                ...
                                if (title === 'Biological Ratios') {
                                  ...
                                  if (label === 'Sodium:Potassium')
                                    ratioStatus =
                                      val <= 1.0 ? 'good' : val <= 2.0 ? 'fair' : 'poor';
                                  if (label === 'Zinc:Copper')
                                    ratioStatus =
                                      val >= 8 && val <= 12
                                        ? 'good'
                                        : val >= 5 && val <= 15
                                          ? 'fair'
                                          : 'poor';
                                  if (label === 'Omega 6:3 Ratio')
                                    ratioStatus =
                                      val <= 4.0 ? 'good' : val <= 10.0 ? 'fair' : 'poor';
                                  if (label === 'Calcium:Magnesium')
                                    ratioStatus =
                                      val >= 1.7 && val <= 2.5
                                        ? 'good'
                                        : val >= 1.5 && val <= 3.0
                                  ...
                                  styles =
                                    ratioStatus === 'good'
                                      ? {
                                          ...
                                          fade: 'bg-emerald-500/5',
                                          textFill: 'text-emerald-500',
                                          bg: 'bg-emerald-500',
                                ...
                                return (
                                  <div
                                    ...
                                    className={cn(
                                      'p-4 rounded-2xl border bg-white dark:bg-slate-950 cursor-pointer hover:shadow-md transition-all relative group',
                                      t.itemBorder,
                                      pct !== null || title === 'Biological Ratios'
                                  ...
                                  >
                                    <p
                                      className={cn(
                                        'text-[8px] uppercase font-black truncate mb-1 flex items-center gap-0.5 whitespace-nowrap overflow-hidden',
                                      )}
                                    >
                                      {title === 'Biological Ratios' && label.includes(':') ? (
                                        <>
                                          <span
                                            className={
                                              label.startsWith('Sodium')
                                                ? 'text-blue-400'
                                                : label.startsWith('Zinc')
                                                  ? 'text-orange-400'
                                                  : label.startsWith('Omega')
                                                    ? 'text-indigo-400'
                                                    : label.startsWith('Calcium') &&
                                          ...
                                          <span
                                            className={
                                              label.endsWith('Potassium')
                                                ? 'text-emerald-400'
                                                : label.endsWith('Copper')
                                                  ? 'text-rose-400'
                                                  : label.includes('3 Ratio')
                                                    ? 'text-purple-400'
                                                    : label.endsWith('Magnesium')
                                                      ? 'text-pink-400'
                                                      : label.endsWith('Phosphorus')
                                                        ? 'text-amber-400'
                                                        : ''
                                            }
                                          >
                                        ...
                                        <>
                                          <div className="flex items-baseline gap-1">
                                            <span
                                              className={cn(
                                                'text-lg font-bold',
                                                title === 'Biological Ratios' ? styles.text : '',
                                              )}
                                            >
                                              {val.toFixed(2)}
                                            </span>
                                          ...
                                          {rda && (
                                            <p className="text-[9px] font-bold text-slate-400 mt-0.5">
                                              Target: {Math.round(rda)}
                                              {unitStr === 'kcal' ? 'kcal' : unitStr}
                                            </p>
                                          )}
                                          {ratioTarget && (
                                            <p className="text-[9px] font-bold text-slate-400 mt-0.5">
                        ...
                        <div className="space-y-6">
                          ...
                          {nutrientsView === 'essential' && (
                            <div className="border border-slate-200 dark:border-slate-800 rounded-2xl p-6 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm animate-in fade-in slide-in-from-top-4">
                              ...
                              <NutrientGrid
                                title="Core Macronutrients"
                                icon={Zap}
                                theme="orange"
                                subtitle="Caloric & Macro Breakdown"
                                breakdownLabels={['Protein', 'Carbs', 'Fat']}
                                items={{
                                  Energy: ['calories'],
                                  Protein: ['protein'],
                                  Carbs: ['carbs'],
                              ...
                              <NutrientGrid
                                ...
                                icon={Zap}
                                theme="indigo"
                                subtitle="Hydration & Mineral Balance"
                                items={{
                              ...
                              <NutrientGrid
                                ...
                                icon={Gem}
                                theme="rose"
                                subtitle="Essential micro-minerals"
                                items={{
                                  Iron: ['Iron', 'iron_mg'],
                                  Zinc: ['Zinc', 'zinc_mg'],
                              ...
                              <NutrientGrid
                                ...
                                icon={Droplet}
                                theme="blue"
                                subtitle="Water-soluble vitamins (B-Complex & C)"
                                items={{
                              ...
                              <NutrientGrid
                                ...
                                icon={Battery}
                                theme="emerald"
                                subtitle="Fat-soluble storage (A, D, E, K)"
                                breakdownLabels={['Vitamin A', 'Vitamin E']}
                                items={{
                          ...
                          {nutrientsView === 'advanced' && (
                            <div className="border border-slate-200 dark:border-slate-800 rounded-2xl p-6 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm animate-in fade-in slide-in-from-top-4">
                              ...
                              <NutrientGrid
                                ...
                                icon={Dna}
                                theme="amber"
                                subtitle="Key nutrient balances for your overall wellbeing"
                                items={{
        ...
        {breakdownNutrient && NUTRIENT_BREAKDOWNS[breakdownNutrient] && plan && (
          <div
            ...
            onClick={() => setBreakdownNutrient(null)}
          >
            <div
              className="bg-background rounded-2xl max-w-lg w-full p-6 shadow-2xl relative animate-in zoom-in-95 fade-in duration-200"
            ...
            >
              ...
              <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-2">
                {NUTRIENT_BREAKDOWNS[breakdownNutrient].map(
                  ({ label, keys, unit, isEssential, hiddenByDefault, isExpandable }) => {
                    // Skip hidden items unless expanded
                    // For now, we assume 'Sugars (Total)' is the parent for all hidden items in Carbs
                    if (hiddenByDefault && !expandedBreakdownSections['Sugars (Total)'])
                      return null;
                        ...
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              {isEssential && (
                                <span className="text-[9px] uppercase font-black tracking-wider bg-black/5 dark:bg-white/10 px-1.5 py-0.5 rounded text-foreground/50">
                                  Essential
                                </span>
                              )}
                            </div>
              ...
              <div className="mt-6 pt-4 border-t border-border">
                <p className="text-xs text-muted-foreground text-center">
                  {breakdownNutrient === 'Vitamin A' &&
                    'Carotenoids (plant-based) are converted to retinol. Beta-carotene is the most efficient precursor.'}
                  {breakdownNutrient === 'Vitamin E' &&
                    'Alpha-tocopherol is the most biologically active form. Other tocopherols have antioxidant properties.'}
                  {breakdownNutrient === 'Protein' &&
                    'Essential amino acids cannot be made by the body and must come from food.'}
                  {breakdownNutrient === 'Carbs' &&
                    'Sugars include natural fruit sugars (fructose) and milk sugars (lactose). Added sugars should be minimized.'}
                  {breakdownNutrient === 'Fat' &&
                    'Unsaturated fats (Mono/Poly) are heart-healthy. Omega-3s are vital for brain & heart health.'}
                </p>
              </div>
            </div>
          </div>
