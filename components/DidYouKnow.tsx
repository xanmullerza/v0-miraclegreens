...
export function DidYouKnow({ phytonutrients, phytonutrientsWithSources, foodName, className }: DidYouKnowProps) {
    ...
    return (
        <div className={cn("p-6 pt-5 rounded-3xl border bg-card border-border mb-6 shadow-sm", className)}>
            <h4 className="font-black flex items-center gap-2 mb-1 uppercase tracking-widest text-[10px] text-amber-500">
                <Leaf className="h-4 w-4" /> PHYTONUTRIENTS
            </h4>
            <div className="text-[10px] text-muted-foreground mb-4 border-b border-border pb-3">
                {isSingleFood ? (
                    <><span className="text-amber-500 font-bold">{foodName}</span> contains powerful plant-based &quot;bodyguards&quot; that protect your cells from stress.</>
                ) : (
                    <>This profile is loaded with plant-based &quot;bodyguards&quot; sourced from multiple ingredients.</>
                )}
            </div>
                            ...
                            {isOpen && (
                                <div className="px-4 pb-4 pt-1 border-t border-border bg-muted/30">
                                    ...
                                    {sources.length > 0 && !isSingleFood && (
                                        <p className="text-[10px] font-bold text-emerald-500 flex items-center gap-1 mt-3 pt-2 border-t border-border">
